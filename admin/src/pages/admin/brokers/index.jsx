...
const Brokers = () => {
  ...
  const create = () => navigate("/brokers/create");
  const edit = (id) => () => navigate(`/brokers/edit/${id}`);

  const categories = useDataFilter({
    type: "categories",
    request: CategoriesService.getAll,
    lang: params.lang
  });
  ...
  const setInitials = handleRequest(async () => {
    const resp = await BrokersService.getAll({
      page: params.page,
      size: params.size,
      lang: params.lang,
              ...
              render={(items) =>
                items.map((item) => (
                  <Tag key={item} color="green">
                    {categories.find((filter) => filter.value === item)?.text}
                  </Tag>
                ))
              }
            />
              ...
              render={(_, record) => (
                <Space size={5} split={<Divider type="vertical" />}>
                  ...
                  <RemoveBoundary
                    ...
                    handler={() => BrokersService.delete(record._id)}
                  >
                    <Button
                      danger
