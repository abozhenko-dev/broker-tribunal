...
const NewsTags = () => {
  ...
  const create = () => navigate("/news-tags/create");
  const edit = (id) => () => navigate(`/news-tags/edit/${id}`);

  const { handleRequest, isLoading } = useRequests();

  const getNewsTags = handleRequest(async () => {
    const resp = await NewsTagsService.getAll({
      page: params.page,
      size: params.size,
      lang: params.lang,
            ...
            <Table.Column
              ...
              render={(_, record) => (
                <Space size={5} split={<Divider type="vertical" />}>
                  ...
                  />
                  <RemoveBoundary
                    onSuccess={() => getNewsTags()}
                    handler={() => NewsTagsService.delete(record._id)}
                  >
                    <Button
                      danger
